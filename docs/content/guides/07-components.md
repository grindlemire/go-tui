...
`{expr}` is reserved for text and must be a `string`, so `{t.content[t.active]}` is a Go compile error. See [Element expressions](../reference/gsx-syntax.md#element-expressions) in the syntax reference.
