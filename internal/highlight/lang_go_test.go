...
func TestLexGo(t *testing.T) {
	...
	tests := map[string]tc{
		"keyword string number comment": {
			code: `var x = 1 // c`,
			want: [][]Token{{
				{KindKeyword, "var"}, {KindPlain, " "},
				{KindPlain, "x"}, {KindPlain, " "},
				{KindOperator, "="}, {KindPlain, " "},
				{KindNumber, "1"}, {KindPlain, " "},
				{KindComment, "// c"},
			}},
		},
		"func name is KindType, param type stays plain": {
			...
			want: [][]Token{{
				...
				{KindType, "Foo"}, {KindOperator, "("},
				{KindPlain, "n"}, {KindPlain, " "},
				{KindPlain, "int"}, {KindOperator, ")"},
			}},
		},
