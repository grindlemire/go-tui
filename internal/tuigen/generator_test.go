...
templ Counter() {
	...
	if !strings.Contains(code, "countText := tui.New(") {
...
templ Text(count int) {
	<span>{fmt.Sprintf("Count: %d", count)}</span>
			...
			wantContains: []string{
				`tui.WithText(fmt.Sprintf("Count: %d", count))`,
			},
		},
	}
...
templ Dashboard(items []Item, selectedIndex int) {
	<div direction={tui.Column} padding=1>
		...
		<div direction={tui.Column} flexGrow=1>
			@for i, item := range items {
				@if i == selectedIndex {
...
func TestGenerator_ScrollableAttribute(t *testing.T) {
	input := `package x
templ ScrollView() {
	<div scrollable={tui.ScrollVertical}>
		<span>content</span>
	</div>
}`
	...
	if !strings.Contains(code, "tui.WithScrollable(tui.ScrollVertical)") {
...
templ Test() {
	...
	// Self-closing element should still generate valid tui.New()
	if !strings.Contains(code, "tui.New()") {
...
templ Test() {
	...
	if !strings.Contains(code, "item := tui.New(") {
...
templ List(items []string) {
	...
	if !strings.Contains(code, "tui.WithText(item)") {
...
func TestGenerator_BooleanAttributes(t *testing.T) {
	input := `package x
templ Test() {
	<div scrollable={tui.ScrollVertical}></div>
}`
...
templ Test() {
	...
	if !strings.Contains(code, "tui.WithFlexGrow(1)") {
		t.Errorf("missing flexGrow\nGot:\n%s", code)
	}

	if !strings.Contains(code, "tui.WithFlexShrink(0)") {
...
templ Card(title string) {
	...
	if !strings.Contains(code, "children []*tui.Element") {
...
templ App() {
	...
	if !strings.Contains(code, "func Card(title string, children []*tui.Element) CardView") {
		t.Errorf("Card should have children parameter and return CardView\nGot:\n%s", code)
	}

	// Check that App creates children slice
	if !strings.Contains(code, "_children := []*tui.Element{}") {
	...
	<div class="flex-col p-2 border">
		<span class="font-bold italic">Title</span>
			...
			wantContains: []string{
				...
				"tui.WithPadding(2)",
				"tui.WithBorder(tui.BorderSingle)",
				"tui.WithTextStyle(tui.NewStyle().Bold().Italic())",
			},
		},
		"alignment classes": {
			input: `package x
...
func TestGenerator_NamedRefs(t *testing.T) {
	...
	tests := map[string]tc{
		"simple named ref": {
			input: `package x
templ StreamBox() {
	<div #Content scrollable={tui.ScrollVertical}></div>
}`,
			wantContains: []string{
				...
				"watchers []tui.Watcher",
				"Content  *tui.Element",
				"var Content *tui.Element",  // Forward-declared
				"Content = tui.New(",        // Assignment, not declaration
				"tui.WithScrollable(tui.ScrollVertical)",
				"view = StreamBoxView{",
				"Root:     Content,",
				"Content:  Content,",
	...
	<div>
		...
		<div #Footer height={3}></div>
			...
			wantContains: []string{
				...
				"Content  *tui.Element",
				"Footer   *tui.Element",
				"var Header *tui.Element",  // Forward-declared
				"var Content *tui.Element", // Forward-declared
				"var Footer *tui.Element",  // Forward-declared
				"Header = tui.New(",        // Assignment
				"Content = tui.New(",       // Assignment
				"Footer = tui.New(",        // Assignment
				"Header:   Header,",
				"Content:  Content,",
				"Footer:   Footer,",
			},
...
templ Sidebar() {
	<nav #Navigation class="flex-col"></nav>
			...
			wantContains: []string{
				...
				"Root       *tui.Element",
				"Navigation *tui.Element",
				"var Navigation *tui.Element", // Forward-declared
				"Navigation = tui.New(",       // Assignment
				"Root:       Navigation,",
...
templ ItemList(items []string) {
	...
	if !strings.Contains(code, "Items []*tui.Element") {
		t.Errorf("missing slice field for loop ref\nGot:\n%s", code)
	}

	// Should declare var at function scope
	if !strings.Contains(code, "var Items []*tui.Element") {
...
templ UserList(users []User) {
	...
	if !strings.Contains(code, "Users    map[string]*tui.Element") {
		t.Errorf("missing map field for keyed ref\nGot:\n%s", code)
	}

	// Should make map at function scope
	if !strings.Contains(code, "Users := make(map[string]*tui.Element)") {
...
templ Counter() {
	...
	if !strings.Contains(code, "tui.WithFocusable(true)") {
